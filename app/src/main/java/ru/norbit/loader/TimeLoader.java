...
/**
 * Created by safaralialisultanov on 06.07.16.
 */
public class TimeLoader extends Loader<String> {
    ...
    public TimeLoader(Context context, Bundle args) {
        ...
        if (args != null)
            format = args.getString(ARGS_TIME_FORMAT);  // достать данные из Bundle
        if (TextUtils.isEmpty(format))                  // проверка на пустоту строки
    ...
    /* Уничтожение лоадера. Вызывается при закрытии Активити или
        фрагмента, к которому привязан Лоадер */
    @Override
